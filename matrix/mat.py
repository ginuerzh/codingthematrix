...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {(r,c):A[r,c]+B[r,c] for r in A.D[0] for c in A.D[1] if A[r,c]+B[r,c] != 0})
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    rd = {row:Vec(M.D[1], {col:M[row,col] for col in M.D[1]}) for row in M.D[0]}
    return sum([v[k]*rd[k] for k in v.D])
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    rd = {col:Vec(M.D[0], {row:M[row,col] for row in M.D[0]}) for col in M.D[1]}
    return sum([v[k]*rd[k] for k in v.D])
